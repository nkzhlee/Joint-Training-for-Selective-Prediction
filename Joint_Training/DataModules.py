...
class SequenceDataset(Dataset):
    def __init__(self, dataset_file_path, tokenizer, device):
        ...
        for file in dataset_file_path:
            with open(file) as csvfile:
                ...
                # Convert string representation of embeddings to actual array objects
                ...
                for row in file_data:
                    ...
                    cat = row[-2]
                    # for GPT embeddings
                    #embedding = ast.literal_eval(row[-1])
                    q_id = row[2]
                    #print(row[0])
                    q_text = q_text_dict[q_id]
                    # col-stat
                    #q_context = q_context_dict[q_id]
                    #embedding = CLS_TOKEN + row[1] + SEP_TOKEN + q_text + SEP_TOKEN + q_context
                    embedding = CLS_TOKEN + row[1] + SEP_TOKEN + q_text
                    # print(refs)
                    data_list = []
                    ref_list = []
                    if len(correct_ref_dict[q_id])>1:
                        ref_list = correct_ref_dict[q_id]
                    else:
                        ref_list.append(correct_ref_dict[q_id][0])
                        ref_list.append(correct_ref_dict[q_id][0])
                    for t in ref_list:
    ...
    def __getitem__(self, index):
        DEVICE = self.device
        label, emb, lines = self.data_dict[index]
        label = self.tag2id[label]
        input_ids, attention_masks = [], []
        p_tokenizer = AutoTokenizer.from_pretrained(hyperparameters['defer_model_name'])
        #for classifier
        for line in lines:
            ...
            # for policy
        tokenized_emb = p_tokenizer(emb, padding="max_length", truncation=True, max_length=hyperparameters['max_length'])
        emd_ids = tokenized_emb["input_ids"]
        emb_attention_masks = tokenized_emb["attention_mask"]
        return {
            "input_ids":  torch.tensor(input_ids, dtype=torch.long, device=self.device),
            "attention_mask": torch.tensor(attention_masks, dtype=torch.long, device=self.device),
            # "embedding":torch.tensor(emb, dtype=torch.long, device=self.device),
            "embedding":torch.tensor(emd_ids, dtype=torch.long, device=self.device),
            "emb_attention_mask": torch.tensor(emb_attention_masks, dtype=torch.long, device=self.device),
            "label": label,
        }
